...
public class SplineControlledParticleSystem : MonoBehaviour {
	...
	void LateUpdate() {
		...
		for (int i = 0; i < aliveParticlesCount; i++) {
			...
			var offshootDirection = Quaternion.AngleAxis(1080 * seedAsPercent, -_spline.GetDirection(travelPercentage, false)) *
			                        _spline.Up(travelPercentage);
			var previousOffshootDirection =
			Quaternion.AngleAxis(1080 * seedAsPercent, -_spline.GetDirection(travelPercentage - _previousDiff, false)) *
			_spline.Up(travelPercentage - _previousDiff, false);

			//cache our positions:
			var position = _spline.GetPosition(travelPercentage, false);
			...
			switch (_particleSystem.main.simulationSpace) {
				case ParticleSystemSimulationSpace.Local:
					...
					previousPosition =
					_particleSystem.transform.InverseTransformPoint(lastPosition + previousOffshootDirection * previousOffset);
					break;
