...
namespace Pixelplacement {
	...
	public class SplineAnchor : MonoBehaviour {
		...
		[SerializeField]
		[HideInInspector]
		Transform _masterTangent;
		[SerializeField]
		[HideInInspector]
		Transform _slaveTangent;
		...
		void TangentChanged() {
			...
			switch (tangentMode) {
				...
				case TangentMode.Mirrored:
					var mirroredOffset = _masterTangent.position - transform.position;
				...
				case TangentMode.Aligned:
					...
					var alignedOffset = (_masterTangent.position - transform.position).normalized;
		...
		public void Tilt(Vector3 angles) {
			...
			var inPosition = InTangent.position;
			var outPosition = OutTangent.position;
