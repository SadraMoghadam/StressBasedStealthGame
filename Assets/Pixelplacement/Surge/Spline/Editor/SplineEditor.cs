...
namespace Pixelplacement {
	...
	public class SplineEditor : Editor {
		...
		static void DrawTools(Transform target) {
			var spline = target.GetComponent<Spline> ();
			...
			for (int i = 0; i < spline.Anchors.Length; i++) {
				...
				var currentAnchor = spline.Anchors[i];
				...
				if (spline.toolScale > 0) {
					...
					if (currentAnchor.OutTangent.gameObject.activeSelf) {
						...
						if (SceneView.currentDrawingSceneView != null) {
							lookRotation = Quaternion.LookRotation((SceneView.currentDrawingSceneView.camera.transform.position -
							                                        currentAnchor.OutTangent.position).normalized);
							Handles.CircleHandleCap(0, currentAnchor.OutTangent.position, lookRotation, spline.toolScale * .65f,
							EventType.Repaint);
							Handles.CircleHandleCap(0, currentAnchor.OutTangent.position, lookRotation, spline.toolScale * .25f,
							EventType.Repaint);
						}
					}
					...
					if (currentAnchor.InTangent.gameObject.activeSelf) {
						...
						if (SceneView.currentDrawingSceneView != null) {
							lookRotation = Quaternion.LookRotation((SceneView.currentDrawingSceneView.camera.transform.position -
							                                        currentAnchor.InTangent.position).normalized);
							Handles.CircleHandleCap(0, currentAnchor.InTangent.position, lookRotation, spline.toolScale * .65f,
							EventType.Repaint);
							Handles.CircleHandleCap(0, currentAnchor.InTangent.position, lookRotation, spline.toolScale * .25f,
							EventType.Repaint);
						}
					}
					...
					if (SceneView.currentDrawingSceneView != null) {
						...
						(SceneView.currentDrawingSceneView.camera.transform.position - currentAnchor.Anchor.position).normalized);
					...
					if (spline.direction == SplineDirection.Forward && i == 0 ||
					    spline.direction == SplineDirection.Backwards && i == spline.Anchors.Length - 1) {
			...
			for (int i = 0; i < spline.Anchors.Length - 1; i++) {
				var startAnchor = spline.Anchors[i];
				var endAnchor = spline.Anchors[i + 1];
				Handles.DrawBezier(startAnchor.Anchor.position, endAnchor.Anchor.position, startAnchor.OutTangent.position,
				endAnchor.InTangent.position, spline.color, null, 2);
			}
		}
	}
}
