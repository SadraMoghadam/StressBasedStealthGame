	...
	public class TweenUtilities : MonoBehaviour {
		...
		public static void GenerateAnimationCurvePropertyCode(AnimationCurve curve) {
			...
			for (int i = 0; i < curve.keys.Length; i++) {
				var currentFrame = curve.keys[i];
				code += "new Keyframe (" + currentFrame.time + "f, " + currentFrame.value + "f, " + currentFrame.inTangent + "f, " +
				        currentFrame.outTangent + "f)";
		...
		public static float LinearInterpolate(float from, float to, float percentage) {
			return
			(to - from) * percentage +
			from; //this approach is needed instead of a simple Mathf.Lerp to accommodate ease cuves that overshoot
		}
		...
		public static Rect LinearInterpolate(Rect from, Rect to, float percentage) {
			...
			LinearInterpolate(from.width, to.width, percentage), LinearInterpolate(from.height, to.height, percentage));
		}
