...
namespace Pixelplacement {
	...
	public class Spline : MonoBehaviour {
		...
		int _previousLength;
		int _slicesPerCurve = 10;
		List<SplineReparam> _splineReparams = new();
		...
		public Color SecondaryColor {
			get {
				var secondaryColor = Color.Lerp(color, Color.black, .2f);
		...
		void Update() {
			...
			if (followers != null && followers.Length > 0 && Anchors.Length >= 2) {
				...
				if (_anchorsChanged || _previousChildCount != transform.childCount || direction != _previousDirection ||
				    loop != _previousLoopChoice) {
		...
		public void CalculateLength() {
			...
			for (int i = 1; i < totalSlices + 1; i++) {
				...
				//position ends:
				var start = GetPosition(previousPercent, false);
				var end = GetPosition(percent, false);
		...
		public Vector3 Up(float percentage, bool normalized = true) {
			var lookRotation = Quaternion.LookRotation(GetDirection(percentage, normalized));
		...
		public Vector3 GetDirection(float percentage, bool normalized = true) {
			...
			//get direction:
			var curveDetail = GetCurve(percentage);

			//avoid an error in editor usage where this index can be -1:
			if (curveDetail.currentCurve < 0) return Vector3.zero;

			var startAnchor = Anchors[curveDetail.currentCurve];
			var endAnchor = Anchors[curveDetail.currentCurve + 1];
			return BezierCurves.GetFirstDerivative(startAnchor.Anchor.position, endAnchor.Anchor.position, startAnchor.OutTangent.position,
			endAnchor.InTangent.position, curveDetail.currentCurvePercentage).normalized;
		}
		...
		public Vector3 GetPosition(float percentage, bool normalized = true) {
			...
			var startAnchor = Anchors[curveDetail.currentCurve];
			var endAnchor = Anchors[curveDetail.currentCurve + 1];
			return BezierCurves.GetPoint(startAnchor.Anchor.position, endAnchor.Anchor.position, startAnchor.OutTangent.position,
			endAnchor.InTangent.position, curveDetail.currentCurvePercentage, true, 100);
		}
		...
		public Vector3 GetPosition(float percentage, Vector3 relativeOffset, bool normalized = true) {
			...
			var lookRotation = Quaternion.LookRotation(GetDirection(percentage));
			...
			//translate position:
			var offset = position + right * relativeOffset.x;
		...
		public GameObject[] AddAnchors(int count) {
			//refs:
			var anchorTemplate = Resources.Load("Anchor") as GameObject;

			//create return array:
			var returnObjects = new GameObject[count];
			...
			for (int i = 0; i < count; i++) {
				...
				//create a new anchor:
				var newAnchor = Instantiate<GameObject>(anchorTemplate);
				newAnchor.name = newAnchor.name.Replace("(Clone)", "");
				var anchor = newAnchor.GetComponent<SplineAnchor> ();
				...
				if (previousPreviousAnchor != null && previousAnchor != null) {
					...
					var direction = (previousAnchor.position - previousPreviousAnchor.position).normalized;
