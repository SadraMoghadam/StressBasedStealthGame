...
namespace Pixelplacement {
	...
	public class InitializationRequirements {
		static InitializationRequirements() {
			//state machines:
			var stateMachines = Resources.FindObjectsOfTypeAll<StateMachine> ();
			foreach (var item in stateMachines) {
				if (item.GetComponent<Initialization> () == null) item.gameObject.AddComponent<Initialization> ();
			}

			//display object:
			var displayObjects = Resources.FindObjectsOfTypeAll<DisplayObject> ();
			...
			foreach (var item in Resources.FindObjectsOfTypeAll<GameObject> ()) {
				foreach (var subItem in item.GetComponents<Component> ()) {
